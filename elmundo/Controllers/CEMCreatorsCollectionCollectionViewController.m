//
//  CEMCreatorsCollectionCollectionViewController.m
//  elmundo
//
...
@interface CEMCreatorsCollectionCollectionViewController (){
...
@implementation CEMCreatorsCollectionCollectionViewController

static NSString * const reuseIdentifier = @"Cell";
- (id)initWithTitle:(NSString *)aTitle
           creators:(NSArray *)arrayOfArticles
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    [[CEMElMundoApi alloc] creators:@"userId"
                                calledBy:self
                             withSuccess:@selector(creatorslDidEnd:)
                              andFailure:@selector(creatorsFailure:)];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == (int)[_tableElements count]-1){
        ...
        [label setText:NSLocalizedString([item objectForKey:@"creator"],nil)];
...
-(void)creatorslDidEnd:(id)result{
    _tableElements = (NSArray *)result;
    [self.collectionView reloadData];
}

-(void)creatorsFailure:(id)result{
    NSLog(@"creatorsFailure");
}
